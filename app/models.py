...
class Cat(db.Model):
    ...
    adoptions = db.relationship('Adoption', back_populates='cat', foreign_keys='[Adoption.cat_id]')

class User(UserMixin, db.Model):
    id = db.Column(db.Integer, primary_key=True)
    username = db.Column(db.String(255), unique=True, nullable=False)
    password = db.Column(db.String(255), nullable=False)
    cats_adopted = db.relationship('Adoption', back_populates='adopter', foreign_keys='[Adoption.user_id]')
...
class Adoption(db.Model):
    ...
    adopter = db.relationship('User', back_populates='cats_adopted', foreign_keys=[user_id])
